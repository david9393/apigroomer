...
func TraerClientes(p models.Cliente) (error, []models.Cliente) {
	...
	db := Pool()
	const exec = `SELECT apellidos, id, nit, nombres, direccion, email, telefono,ispredeterminado,idvendedor
	FROM public.t01_cliente where  ($1 =0 OR id = $1) `
	...
	for rows.Next() {
		...
		err = rows.Scan(&cliente.Apellidos, &cliente.Id, &cliente.Nit, &cliente.Nombres,
			&cliente.Direccion, &cliente.Email, &cliente.Telefono, &cliente.IsPrederminado, &cliente.IdVendedor)
		if err != nil {
			log.Fatal(err)
			return err, listClientes
		}
...
func CrearCliente(c models.Cliente) error {
	...
	const exec = `INSERT INTO public.t01_clientes(
		apellidos,nit, nombres, direccion, email, telefono, ispredeterminado,idvendedor)
...
func parametrosClientes(c models.Cliente) []interface{} {
	...
	args = append(args, &c.IsPrederminado)
